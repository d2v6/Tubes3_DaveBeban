...
class SQLSeeder:
    ...
    def _count_insert_records(self, insert_command):
        ...
        try:
            ...
            values_part = insert_command.split('VALUES', 1)[1] if 'VALUES' in insert_command else ""
            # Simple count of opening parentheses after VALUES
            return values_part.count('(') - values_part.count('((')  # Subtract nested parentheses
        except:
            return "Unknown"
    ...
    def _verify_seeding(self):
        ...
        try:
            ...
            profile_count = self.db.execute_query("SELECT COUNT(*) as count FROM ApplicantProfile")
            if profile_count:
                print(f"   ApplicantProfile: {profile_count[0]['count']} records")
            
            # Check ApplicationDetail table
            detail_count = self.db.execute_query("SELECT COUNT(*) as count FROM ApplicationDetail")
            if detail_count:
                print(f"   ApplicationDetail: {detail_count[0]['count']} records")
            
            # Check sample data
            sample_profiles = self.db.execute_query("SELECT first_name, last_name FROM ApplicantProfile LIMIT 5")
            if sample_profiles:
